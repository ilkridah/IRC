...
void ChannelCmds::Joining_Channel(const string& Channelname,
                                  const string& user,
                                  const string& key) {
...
void ChannelCmds::Leaving_Channel(const string& Channelname,
                                  const string& user) {
    ...
    // std::find(Chlist[Channelname]->users.begin(),
    // Chlist[Channelname]->users.end(), user);
    if (_channel_handler.is_member(Channelname, user))
    ...
    //     Chlist[Channelname]->admins.find(user); if (iter !=
    //     Chlist[Channelname]->admins.end())
    //         Chlist[Channelname]->admins.erase(iter);
    //     if (Chlist[Channelname]->admins.size()== 0 &&
    //     Chlist[Channelname]->users.size() != 0)
...
// bool ChannelCmds::is_channel_admin(string Channelname, string user) {  //
// chan exist
//     std::set<string>::iterator it =
//     Chlist[Channelname]->admins.find(user); if (it !=
//     Chlist[Channelname]->admins.end())
//         return 1;
//     return 0;
// }
...
// void ChannelCmds::add_channel_guest(std::string Channelname, std::string
// user)
// {
